...
def run(pipeline_options, known_args):
  ...
  with impl.Context(known_args.transform_temp_dir):
    articles = (
      ...
      project=gcp_project, query=get_source_query(known_args.limit), use_standard_sql=True))
    )
    ...
    (
        articles
        | "Convert to entity" >> beam.Map(lambda input_features: create_entity(input_features, known_args.kind))
